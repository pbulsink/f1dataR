...
plot_fastest <- function(
  ...
  driver_data <- load_driver_telemetry(
    season,
    round,
    session,
    driver_abbreviation,
  ...
  if (color == "gear") {
    fastplot <- ggplot2::ggplot(
      driver_data,
      ggplot2::aes(.data$x, .data$y, color = as.factor(.data$n_gear), group = 1)
    ) +
    ...
    fastplot <- ggplot2::ggplot(
      driver_data,
      ggplot2::aes(.data$x, .data$y, color = .data$speed, group = 1)
    ) +
      ggplot2::geom_path(linewidth = 4, lineend = "round") +
...
correct_track_ratio <- function(
  ...
  if (!requireNamespace("ggplot2", quietly = TRUE)) {
    cli::cli_abort(
      "f1dataR::correct_track_ratio() requires ggplot2 package installation"
    )
  }
  ...
  maxdiff <- max(
    abs(xrange[2] - xrange[1]),
    abs(yrange[2] - yrange[1]),
    na.rm = TRUE
  )
