test_that("load_constructors works", {
  ...
  if (dir.exists(file.path(tempdir(), "tst_load_constructors"))) {
    unlink(
      file.path(tempdir(), "tst_load_constructors"),
      recursive = TRUE,
      force = TRUE
    )
  }
  ...
  withr::local_options(
    f1dataR.cache = file.path(tempdir(), "tst_load_constructors")
  )
...
test_that("load_constructors works without internet", {
  ...
  if (dir.exists(file.path(tempdir(), "tst_load_constructors2"))) {
    unlink(
      file.path(tempdir(), "tst_load_constructors2"),
      recursive = TRUE,
      force = TRUE
    )
  }
  ...
  withr::local_options(
    f1dataR.cache = file.path(tempdir(), "tst_load_constructors2")
  )
