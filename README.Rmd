...
Note the Ergast Motor Racing Database API was shut down at the end of 2024. A new data source (Jolpica-F1 project) was identified and implemented.
...
### Driver Telemetry
> `load_driver_telemetry(season = "current", round = "last", session = "R", driver, laps = "fastest")`
...
> `load_session_laps(season = "current", round = "last", session = "R", add_weather = FALSE)`
...
### Plotting
> `plot_fastest(season = "current", round = 1, session = "R", driver, color = "gear")`

A built in plotting function that plots a driver's fastest lap around the circuit, shaded by `speed` or `gear`.
...
### Metadata Lookups
The package echoes the metadata information look-up from the FastF1 package. This is a convenient way to programmatically look up drivers, teams, driver-team relationships, team colors, driver colors, tire types & colors and more. See the following functions for this look-up:
